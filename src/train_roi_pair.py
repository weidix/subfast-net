...
from .train_roi import format_dataset_summary, model_parameter_count, parse_roi_size, seed_everything, synchronize_device


def make_dataset(settings: RoiPairTrainSettings, *, train: bool) -> RoiPresenceEmbeddingDataset:
...
def assert_disjoint_datasets(
    train_dataset: RoiPresenceEmbeddingDataset,
    val_dataset: RoiPresenceEmbeddingDataset,
) -> None:
...
def require_pair_classes(selection: EmbeddingPairSelection, *, name: str) -> None:
...
def load_scheduled_pair_batch(
    dataset: RoiPresenceEmbeddingDataset,
    scheduled: ScheduledEmbeddingBatch,
) -> Any:
    return collate_roi_batch([dataset[index] for index in scheduled.sample_indices])
...
class ScheduledPairBatchDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple[ScheduledEmbeddingBatch, Any]:
...
def run_training(settings: RoiPairTrainSettings) -> dict[str, float]:
    ...
    train_pair_pools = build_embedding_pair_pools(
