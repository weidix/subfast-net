...
@dataclass(frozen=True)
class ScheduledPairBatch:
    sample_indices: tuple[int, ...]
    pairs: tuple[RoiPair, ...]


@dataclass(frozen=True)
class RoiPairEpochSchedule:
...
@dataclass(frozen=True)
class RoiPairPools:
    positive_pairs: tuple[RoiPair, ...]
    local_negative_pairs: tuple[RoiPair, ...]
    ocr_negative_pairs: Sequence[RoiPair]
    total_positive_roi_count: int
...
class LazyOcrNegativePairPool(Sequence[RoiPair]):
    ...
    def sample(self, count: int, rng: random.Random) -> list[RoiPair]:
    ...
    def _pair_at_rank(self, rank: int) -> RoiPair:
        ...
        return RoiPair(i=i, j=j, same=False, source="ocr")
    ...
    *,
    ...
    for offset, i in enumerate(positive_indices):
        for j in positive_indices[offset + 1 :]:
            ...
            if same:
                pairs.append(RoiPair(i=i, j=j, same=True, source="local"))
                seen.add((i, j))
            ...
            if adjacent:
                if is_same_subtitle_text(normalized_ocr_texts[i], normalized_ocr_texts[j]):
                    skipped_pairs += 1
                    continue
                pairs.append(RoiPair(i=i, j=j, same=False, source="local"))
                seen.add((i, j))
            ...
            if (
                ...
                if normalized_ocr_text_similarity_at_most(
                    ...
                    ocr_candidates.append(RoiPair(i=i, j=j, same=False, source="ocr"))
                    continue
...
def build_pair_epoch_schedule(
    ...
    *,
    ...
    seed: int,
    epoch: int,
    pair_pools: RoiPairPools | None = None,
    ...
    pools = pair_pools or build_pair_pools(
        samples,
...
def build_pair_pools(
    ...
    *,
    ...
    positive_pairs: list[RoiPair] = []
    local_negative_pairs: list[RoiPair] = []
    ...
    if (
        ...
        ocr_negative_pool: Sequence[RoiPair] = LazyOcrNegativePairPool(ocr_group_pairs)
    else:
        ocr_negative_pool = ()
...
def _local_negative_pool_satisfies_ratio(
    positive_pairs: Sequence[RoiPair],
    local_negative_pairs: Sequence[RoiPair],
...
def _append_pool_pair(
    ...
    if key in seen:
        return
    seen.add(key)
    pool.append(RoiPair(i=i, j=j, same=same, source=source))


def _with_pair_id(pair: RoiPair, samples: Sequence[object]) -> RoiPair:
    if pair.pair_id:
        return pair
    return RoiPair(
        i=pair.i,
...
def _pair_sort_key(samples: Sequence[object], pair: RoiPair) -> tuple[str, str, str, str, str]:
    left = samples[pair.i]
...
def _select_positive_pairs_for_coverage(
    ...
    *,
    ...
    selected: list[RoiPair] = []
    deferred: list[RoiPair] = []
    selected_pairs: set[RoiPair] = set()

    widest_by_segment: dict[tuple[str, str], RoiPair] = {}
...
def _pair_frame_span(samples: Sequence[object], pair: RoiPair) -> int:
...
def _local_negative_priority(samples: Sequence[object], pair: RoiPair) -> tuple[float, int]:
...
def _take_pairs(pool: Sequence[RoiPair], count: int, rng: random.Random) -> list[RoiPair]:
    if count <= 0 or not pool:
        return []
...
def _pack_pair_batches(pairs: list[RoiPair], batch_size: int) -> list[ScheduledPairBatch]:
    batches: list[ScheduledPairBatch] = []
    current_pairs: list[RoiPair] = []
...
def _make_scheduled_batch(sample_indices: list[int], pairs: list[RoiPair]) -> ScheduledPairBatch:
    ...
    return ScheduledPairBatch(sample_indices=tuple(sample_indices), pairs=local_pairs)
