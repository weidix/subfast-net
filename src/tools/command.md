...
数据生成和标注审阅工具是位于 `src/tools/` 的独立一级包，统一使用
`subfast-tools` 命令：

```bash
uv run subfast-tools --help
uv run subfast-tools build-samples --help
uv run subfast-tools synthesize-samples --help
uv run subfast-tools labels-to-via --help
uv run subfast-tools via-to-labels --help
```
...
```powershell
uv run subfast-tools build-samples data\xxx.mp4 -o data\generated_samples --boxed-images --filter-region 0,600,1280,720
```
...
```powershell
uv run subfast-tools build-samples data\xxx.mp4 -o data\generated_samples --start-frame 3000 --boxed-images --filter-region 0,600,1280,720
```
...
```powershell
uv run subfast-tools build-samples data\xxx.mp4 -o data\generated_samples --det-model-name PP-OCRv4_server_det
```

使用 FFmpeg 视频解码后端示例：

```powershell
uv run subfast-tools build-samples data\xxx.mp4 -o data\generated_samples --video-backend ffmpeg
```
...
uv run subfast-tools labels-to-via --labels-dir data\generated_samples\labels --images-dir data\generated_samples\images --annotations data\generated_samples\annotations.jsonl -o data\generated_samples\via_annotations.json

# VGG Image Annotator JSON -> labels
uv run subfast-tools via-to-labels data\generated_samples\via_annotations.json --labels-dir data\generated_samples\labels
```
...
```text
src\tools\filter_region_helper.html
```

选框审阅和屏蔽工具：

```powershell
uv run subfast-tools review-labels --samples-dir data\generated_samples
```
