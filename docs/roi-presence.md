...
`subfast-roi-presence` 是独立的 ROI 字幕存在性训练命令。V3 checkpoint 只包含 presence 专属 backbone、字幕区域 head 与连续区域证据聚合器。
...
uv run subfast-roi-presence \
  ...
  --output-dir outputs/roi_presence \
  --resize-roi 256x32 \
  --epochs 10 \
  --batch-size 16
```
...
```bash
--resize-roi 256x32 --resize-mode letterbox
```
...
验收口径固定为 Apple M4、PyTorch 2.12.1、FP32、batch 1、`256×32`、显式 valid mask、输入已在 MPS；40 次 warmup、500 次计时，每次 forward 都同步，不含图片读取、resize、传输、sigmoid 或 CPU 读取。
...
```bash
PYTORCH_ENABLE_MPS_FALLBACK=0 uv run subfast-roi-presence-benchmark --device mps
```
