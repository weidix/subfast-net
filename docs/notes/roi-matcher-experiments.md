...
```mermaid
flowchart LR
    A["ROI A: 3×64×256"] --> AV["模型内 HSV Value"]
    B["ROI B: 3×64×256"] --> BV["模型内 HSV Value"]
    AV --> C["严格对称 pair 特征"]
    BV --> C
    C --> C1["共同外观: (A+B)/2"]
    C --> C2["原始变化: abs(A-B)"]
...
- 模型内部撤销输入标准化后只保留 HSV Value；hue/saturation 不进入 pair 特征，不依赖颜色增强或部署预处理。
...
当前颜色不变产物的独立复测为 median `0.574 ms`、P90 `0.682 ms`。计时仅包含 forward，输入和输出均留在 MPS；不包含 I/O、resize、设备传输、sigmoid、阈值或 CPU 读取。现有 checkpoint 完整验证为 `2,724/2,724` 正确，`min positive=0.987896`、`max negative=0.026171`。
...
`0.977785`。该历史 RGB 推理 checkpoint 为 90,533 bytes，模型 tensor 为 70,664 bytes，共 17,074 个可训练参数。
`pair_gap` 基于 sigmoid 概率，旧 `embedding_gap` 基于 cosine；两者可以比较是否完全分离和错误数，
但不能把 gap 的绝对数值当作同一量纲直接相除。

### 历史 RGB 架构的额外 leave-one-video-out 检查
...
|---|---:|---:|---:|
| MPS median | 1.609 ms | 3.390 ms | **0.574 ms** |
| MPS P90 | 2.877 ms | 5.129 ms | **0.682 ms** |
| CPU median | 5.756 ms | 5.804 ms | **0.865 ms** |
| full-data epoch median / best epoch | 81.75 s | 97.43 s | **51.81 s** |
...
| **Direct Pair** | **16,210** | **67,208 B** | **87,141 B** | **0 B** |
...
```mermaid
flowchart TD
    A["同段正对、相邻段难负对、OCR 强负对"] --> B["每轮 9608 正对 + 6726 local 负对 + 2882 OCR 负对"]
    B --> C["两个 ROI 独立轻微光度扰动"]
    C --> D["模型内 HSV Value + 对称共同外观 / 原始变化 / 高频变化"]
    D --> E["16.2k 参数 CNN，端到端更新"]
    E --> F["字幕区域 mask BCE"]
...
读图要点：历史 Presence + Embedding 训练默认使用 32 个 width token 和 256 维单向量 embedding；空 ROI 不参加 embedding pair；memory 只保存本 epoch 先前 batch 的 detached 向量，并在下个 epoch 重建；验证中的 FP/FN 是 pair 数，不是 ROI 样本数。matcher 当前入口见上文。
...
| P1 | embedding 阶段冻结 backbone | 事实 | width/local 的新 checkpoint 实际都冻结了随机初始化 backbone；Direct Pair 已删除该训练方式并端到端更新全部 16,210 参数。 |
...
4. 若未来部署场景需要“一张 ROI 对很多候选”的 descriptor 检索，应重新设计并验证独立实现；不要恢复已删除的旧 embedding 路径。当前相邻帧的一对一判断使用 Direct Pair。
...
- 质量稳定性门槛：历史 RGB 架构的 2026、2027、2028 三个 seed 均通过固定阈值质量门槛；颜色不变架构 v2 当前只复测了正式 seed 2028 checkpoint。
- 泛化门槛：历史 RGB 架构的 leave-one-video-out `roi_samples2` 仍有 9 FN；颜色不变架构尚未重做。本文“最佳”只表示当前仓库正式 held-out 数据上的已验证结果，不能外推成所有视频的零错误保证。
